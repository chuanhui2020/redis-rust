...
> **优化进度**：已完成 4 项（第一批 commit 089cb17），剩余 38 项待实施。
...
| # | 项目 | 级别 | 预估耗时 |
|---|------|------|----------|
| 1 | RDB 写入加 fsync | P0 | 5min |
| 2 | connection.rs UTF-8 安全 | P3 | 5min |
| 3 | scripting panic→error | P3 | 10min |
| 4 | Slowlog 双锁合并 | P3 | 10min |
| 5 | Eviction policy 改 AtomicU8 | P1 | 10min |

#### 第三批（小工作量，可并行）

| # | 项目 | 级别 | 预估耗时 |
|---|------|------|----------|
| 6 | Keyspace 通知 config 免 clone | P1 | 20min |
| 7 | 命令解析零分配 | P1 | 30min |
| 8 | RESP buffer 回滚优化 | P1 | 30min |
| 9 | ACL 密码 SHA1→SHA256 | P0 | 20min |
| 10 | is_expired() 缓存时间戳 | P1 | 20min |
| 11 | Inline 命令解析栈分配 | P3 | 20min |
...
|---|------|------|----------|
| 12 | Lua VM 池化 | P1 | 2h |
| 13 | SCAN cursor 惰性迭代 | P1 | 2h |
| 14 | RwLock unwrap 统一处理 | P0 | 1.5h |
| 15 | blocking_waiters per-shard | P2 | 1.5h |
| 16 | Replication backlog 环形缓冲区 | P1 | 2h |
| 17 | ACL 权限检查 ArcSwap | P1 | 1.5h |
...
| 项目 | 说明 | 涉及文件 | 工作量 |
|------|------|----------|--------|
| scripting panic→error | `scripting.rs:1090,1139` Lua 返回非预期类型时直接 `panic!()`，用户脚本可 crash 服务器。改为返回 Redis error response | scripting.rs | 极小 |
