...
impl CommandExecutor {
    ...
    pub(crate) fn build_debug_object_info(&self, key: &str) -> Result<String> {
        ...
        let refcount = self.storage.object_refcount(key)?.unwrap_or(0);
        let idletime = self.storage.object_idletime(key)?.unwrap_or(0);
    ...
    /// Emit AOF, keyspace notification, and replication for a write already applied in storage.
    pub(crate) fn record_applied_write(&self, cmd: &Command) {
    ...
    pub fn execute(&self, cmd: Command) -> Result<RespValue> {
        ...
        // 只在真正需要 AOF、keyspace 通知或复制传播时才 clone 命令
        // keyspace_notifier: 默认 flags==0 时不产生通知，可以跳过 clone
        let need_aof = is_write && self.aof.is_some();
        let need_keyspace = self
            .keyspace_notifier
            .as_ref()
            .is_some_and(|n| !n.config().is_empty());
        let need_replica = is_write
            && self.replication.as_ref().is_some_and(|r| {
                matches!(r.get_role(), crate::replication::ReplicationRole::Master)
                    && r.has_connected_replicas()
            });
        let cmd_for_side_effects = if need_aof || need_keyspace || need_replica {
            Some(cmd.clone())
        } else {
            None
        };
        ...
        // 命令执行成功后再持久化、发送 Keyspace 通知并广播到副本
        if result.is_ok()
            ...
            && duration_us >= slowlog.threshold_us()
        {
            let (cmd_name, args) = extract_cmd_info(
                cmd_for_side_effects
                    .as_ref()
                    .unwrap_or(&Command::Unknown(String::new())),
            );
    ...
    fn do_execute(&self, cmd: Command) -> Result<RespValue> {
        match cmd {
            ...
            Command::Set(key, value, options) => {
                executor_string::execute_set(self, key, value, options)
            }
            Command::SetEx(key, value, ttl_ms) => {
                executor_string::execute_set_ex(self, key, value, ttl_ms)
            }
            ...
            Command::Ttl(key) => {
                ...
                let ttl_sec = if ttl_ms >= 0 { ttl_ms / 1000 } else { ttl_ms };
            ...
            Command::MemoryUsage(key, _samples) => {
                executor_admin::execute_memory_usage(self, key, _samples)
            }
            ...
            Command::Hello(_protover, _auth, _setname) => {
                executor_admin::execute_hello(self, _protover, _auth, _setname)
            }
            ...
            Command::SetBit(key, offset, value) => {
                executor_bitmap::execute_set_bit(self, key, offset, value)
            }
            Command::GetBit(key, offset) => executor_bitmap::execute_get_bit(self, key, offset),
            Command::BitCount(key, start, end, is_byte) => {
                executor_bitmap::execute_bit_count(self, key, start, end, is_byte)
            }
            Command::BitOp(op, destkey, keys) => {
                executor_bitmap::execute_bit_op(self, op, destkey, keys)
            }
            Command::BitPos(key, bit, start, end, is_byte) => {
                executor_bitmap::execute_bit_pos(self, key, bit, start, end, is_byte)
            }
            ...
            Command::XAdd(key, id, fields, nomkstream, max_len, min_id) => {
                executor_stream::execute_x_add(self, key, id, fields, nomkstream, max_len, min_id)
            }
            Command::XLen(key) => executor_stream::execute_x_len(self, key),
            Command::XRange(key, start, end, count) => {
                executor_stream::execute_x_range(self, key, start, end, count)
            }
            Command::XRevRange(key, end, start, count) => {
                executor_stream::execute_x_rev_range(self, key, end, start, count)
            }
            Command::XTrim(key, strategy, threshold) => {
                executor_stream::execute_x_trim(self, key, strategy, threshold)
            }
            Command::XDel(key, ids) => executor_stream::execute_x_del(self, key, ids),
            Command::XRead(keys, ids, count) => {
                executor_stream::execute_x_read(self, keys, ids, count)
            }
            Command::XSetId(key, id) => executor_stream::execute_x_set_id(self, key, id),
            Command::XGroupCreate(key, group, id, mkstream) => {
                executor_stream::execute_x_group_create(self, key, group, id, mkstream)
            }
            Command::XGroupDestroy(key, group) => {
                executor_stream::execute_x_group_destroy(self, key, group)
            }
            Command::XGroupSetId(key, group, id) => {
                executor_stream::execute_x_group_set_id(self, key, group, id)
            }
            Command::XGroupDelConsumer(key, group, consumer) => {
                executor_stream::execute_x_group_del_consumer(self, key, group, consumer)
            }
            Command::XGroupCreateConsumer(key, group, consumer) => {
                executor_stream::execute_x_group_create_consumer(self, key, group, consumer)
            }
            Command::XReadGroup(group, consumer, keys, ids, count, noack) => {
                executor_stream::execute_x_read_group(
                    self, group, consumer, keys, ids, count, noack,
                )
            }
            ...
            Command::XClaim(key, group, consumer, min_idle, ids, justid) => {
                executor_stream::execute_x_claim(self, key, group, consumer, min_idle, ids, justid)
            }
            Command::XAutoClaim(key, group, consumer, min_idle, start, count, justid) => {
                executor_stream::execute_x_auto_claim(
                    self, key, group, consumer, min_idle, start, count, justid,
                )
            }
            Command::XPending(key, group, start, end, count, consumer) => {
                executor_stream::execute_x_pending(self, key, group, start, end, count, consumer)
            }
            Command::XInfoStream(key, full) => {
                executor_stream::execute_x_info_stream(self, key, full)
            }
            Command::XInfoGroups(key) => executor_stream::execute_x_info_groups(self, key),
            Command::XInfoConsumers(key, group) => {
                executor_stream::execute_x_info_consumers(self, key, group)
            }
            ...
            Command::GeoDist(key, member1, member2, unit) => {
                executor_geo::execute_geo_dist(self, key, member1, member2, unit)
            }
            ...
            Command::GeoSearch(
                ...
                withdist,
                withhash,
            ) => executor_geo::execute_geo_search(
                self, key, center_lon, center_lat, by_radius, by_box, order, count, withcoord,
                withdist, withhash,
            ),
            Command::GeoSearchStore(
                ...
                count,
                storedist,
            ) => executor_geo::execute_geo_search_store(
                self,
                destination,
                source,
                center_lon,
            ...
            | Command::ClientReply(_)
            | Command::ClientUnblock(_, _)
            ...
            Command::EvalSha(sha1, keys, args) => {
                executor_admin::execute_eval_sha(self, sha1, keys, args)
            }
            ...
            Command::ObjectEncoding(key) => match self.storage.object_encoding(&key)? {
                Some(enc) => Ok(RespValue::BulkString(Some(Bytes::from(enc)))),
                None => Ok(RespValue::BulkString(None)),
            },
            Command::ObjectRefCount(key) => match self.storage.object_refcount(&key)? {
                Some(count) => Ok(RespValue::Integer(count)),
                None => Ok(RespValue::BulkString(None)),
            },
            Command::ObjectIdleTime(key) => match self.storage.object_idletime(&key)? {
                Some(secs) => Ok(RespValue::Integer(secs as i64)),
                None => Ok(RespValue::BulkString(None)),
            },
            Command::ObjectFreq(key) => match self.storage.object_freq(&key)? {
            ...
            Command::RandomKey => match self.storage.randomkey()? {
            ...
            Command::ExpireAt(key, timestamp) => {
                executor_admin::execute_expire_at(self, key, timestamp)
            }
            Command::PExpireAt(key, timestamp) => {
                executor_admin::execute_p_expire_at(self, key, timestamp)
            }
            ...
            Command::Lcs(key1, key2, len, idx, _minmatchlen, withmatchlen) => {
                executor_string::execute_lcs(self, key1, key2, len, idx, _minmatchlen, withmatchlen)
            }
            ...
            Command::Rpoplpush(source, dest) => match self.storage.rpoplpush(&source, &dest)? {
                Some(value) => Ok(RespValue::BulkString(Some(value))),
                None => Ok(RespValue::BulkString(None)),
            },
            Command::Lmpop(keys, left, count) => {
                executor_list::execute_lmpop(self, keys, left, count)
            }
            Command::Sort(
                key,
                by_pattern,
            ...
            Command::SortRo(
                ...
                asc,
                alpha,
            ) => executor_admin::execute_sort_ro(
                self,
                key,
                by_pattern,
                get_patterns,
            ...
            Command::Copy(source, destination, replace) => {
                executor_admin::execute_copy(self, source, destination, replace)
            }
            ...
            Command::Restore(key, ttl_ms, serialized, replace) => {
                executor_admin::execute_restore(self, key, ttl_ms, serialized, replace)
            }
            ...
            Command::SetExCmd(key, value, seconds) => {
                executor_string::execute_set_ex_cmd(self, key, value, seconds)
            }
            Command::PSetEx(key, value, ms) => {
                executor_string::execute_p_set_ex(self, key, value, ms)
            }
            ...
            Command::IncrByFloat(key, delta) => {
                executor_string::execute_incr_by_float(self, key, delta)
            }
            Command::SetRange(key, offset, value) => {
                executor_string::execute_set_range(self, key, offset, value)
            }
            Command::GetRange(key, start, end) => {
                executor_string::execute_get_range(self, key, start, end)
            }
            ...
            Command::LRange(key, start, stop) => {
                executor_list::execute_l_range(self, key, start, stop)
            }
            Command::LIndex(key, index) => executor_list::execute_l_index(self, key, index),
            Command::LSet(key, index, value) => {
                executor_list::execute_l_set(self, key, index, value)
            }
            Command::LInsert(key, pos, pivot, value) => {
                executor_list::execute_l_insert(self, key, pos, pivot, value)
            }
            Command::LRem(key, count, value) => {
                executor_list::execute_l_rem(self, key, count, value)
            }
            Command::LTrim(key, start, stop) => {
                executor_list::execute_l_trim(self, key, start, stop)
            }
            Command::LPos(key, value, rank, count, maxlen) => {
                executor_list::execute_l_pos(self, key, value, rank, count, maxlen)
            }
            ...
            Command::BLmove(source, dest, left_from, left_to, _timeout) => {
                ...
                if let Ok(Some(value)) = self.storage.lmove(&source, &dest, left_from, left_to) {
                    self.record_applied_write(&Command::Lmove(
                        source.clone(),
                        dest.clone(),
                        left_from,
                        left_to,
            ...
            Command::BLmpop(keys, left, count, _timeout) => {
                executor_list::execute_b_lmpop(self, keys, left, count, _timeout)
            }
            Command::BRpoplpush(source, dest, _timeout) => {
                // 非阻塞版本：直接尝试移动（用于事务和 AOF 重放）
                if let Ok(Some(value)) = self.storage.rpoplpush(&source, &dest) {
                    self.record_applied_write(&Command::Rpoplpush(source.clone(), dest.clone()));
                    Ok(RespValue::BulkString(Some(value)))
                } else {
                    Ok(RespValue::BulkString(None))
                }
            ...
            Command::HIncrBy(key, field, delta) => {
                executor_hash::execute_h_incr_by(self, key, field, delta)
            }
            Command::HIncrByFloat(key, field, delta) => {
                executor_hash::execute_h_incr_by_float(self, key, field, delta)
            }
            Command::HKeys(key) => executor_hash::execute_h_keys(self, key),
            Command::HVals(key) => executor_hash::execute_h_vals(self, key),
            Command::HSetNx(key, field, value) => {
                executor_hash::execute_h_set_nx(self, key, field, value)
            }
            Command::HRandField(key, count, with_values) => {
                executor_hash::execute_h_rand_field(self, key, count, with_values)
            }
            Command::HScan(key, cursor, pattern, count) => {
                executor_hash::execute_h_scan(self, key, cursor, pattern, count)
            }
            Command::HExpire(key, fields, seconds) => {
                executor_hash::execute_h_expire(self, key, fields, seconds)
            }
            Command::HPExpire(key, fields, ms) => {
                executor_hash::execute_h_p_expire(self, key, fields, ms)
            }
            Command::HExpireAt(key, fields, ts) => {
                executor_hash::execute_h_expire_at(self, key, fields, ts)
            }
            Command::HPExpireAt(key, fields, ts) => {
                executor_hash::execute_h_p_expire_at(self, key, fields, ts)
            }
            ...
            Command::HExpireTime(key, fields) => {
                executor_hash::execute_h_expire_time(self, key, fields)
            }
            Command::HPExpireTime(key, fields) => {
                executor_hash::execute_h_p_expire_time(self, key, fields)
            }
            Command::HPersist(key, fields) => executor_hash::execute_h_persist(self, key, fields),
            Command::HGetDel(key, fields) => executor_hash::execute_h_get_del(self, key, fields),
            Command::HGetEx(key, opt, fields) => {
                executor_hash::execute_h_get_ex(self, key, opt, fields)
            }
            Command::HSetEx(key, seconds, pairs) => {
                executor_hash::execute_h_set_ex(self, key, seconds, pairs)
            }
            ...
            Command::SRandMember(key, count) => {
                executor_set::execute_s_rand_member(self, key, count)
            }
            Command::SMove(source, destination, member) => {
                executor_set::execute_s_move(self, source, destination, member)
            }
            Command::SInterStore(destination, keys) => {
                executor_set::execute_s_inter_store(self, destination, keys)
            }
            Command::SUnionStore(destination, keys) => {
                executor_set::execute_s_union_store(self, destination, keys)
            }
            Command::SDiffStore(destination, keys) => {
                executor_set::execute_s_diff_store(self, destination, keys)
            }
            Command::SScan(key, cursor, pattern, count) => {
                executor_set::execute_s_scan(self, key, cursor, pattern, count)
            }
            ...
            Command::ZUnionStore(destination, keys, weights, aggregate) => {
                executor_zset::execute_z_union_store(self, destination, keys, weights, aggregate)
            }
            Command::ZInterStore(destination, keys, weights, aggregate) => {
                executor_zset::execute_z_inter_store(self, destination, keys, weights, aggregate)
            }
            Command::ZScan(key, cursor, pattern, count) => {
                executor_zset::execute_z_scan(self, key, cursor, pattern, count)
            }
            Command::ZRangeByLex(key, min, max) => {
                executor_zset::execute_z_range_by_lex(self, key, min, max)
            }
            Command::ZRemRangeByLex(key, min, max) => {
                executor_zset::execute_z_rem_range_by_lex(self, key, min, max)
            }
            Command::ZRemRangeByRank(key, start, stop) => {
                executor_zset::execute_z_rem_range_by_rank(self, key, start, stop)
            }
            Command::ZRemRangeByScore(key, min, max) => {
                executor_zset::execute_z_rem_range_by_score(self, key, min, max)
            }
            ...
            Command::ZInterCard(keys, limit) => {
                executor_zset::execute_z_inter_card(self, keys, limit)
            }
            ...
            Command::ZRandMember(key, count, with_scores) => {
                executor_zset::execute_z_rand_member(self, key, count, with_scores)
            }
            Command::ZDiff(keys, with_scores) => {
                executor_zset::execute_z_diff(self, keys, with_scores)
            }
            Command::ZDiffStore(destination, keys) => {
                executor_zset::execute_z_diff_store(self, destination, keys)
            }
            Command::ZInter(keys, weights, aggregate, with_scores) => {
                executor_zset::execute_z_inter(self, keys, weights, aggregate, with_scores)
            }
            Command::ZUnion(keys, weights, aggregate, with_scores) => {
                executor_zset::execute_z_union(self, keys, weights, aggregate, with_scores)
            }
            Command::ZRangeStore(
                dst,
                src,
                ...
                limit_offset,
                limit_count,
            ) => executor_zset::execute_z_range_store(
                self,
                dst,
                src,
                min,
            ...
            Command::ZMpop(keys, min_or_max, count) => {
                executor_zset::execute_z_mpop(self, keys, min_or_max, count)
            }
            Command::ZRevRangeByScore(key, max, min, with_scores, limit_offset, limit_count) => {
                executor_zset::execute_z_rev_range_by_score(
                    self,
                    key,
                    max,
                    min,
            ...
            Command::ZRevRangeByLex(key, max, min, limit_offset, limit_count) => {
                executor_zset::execute_z_rev_range_by_lex(
                    self,
                    key,
                    max,
                    min,
            ...
            Command::ZLexCount(key, min, max) => {
                executor_zset::execute_z_lex_count(self, key, min, max)
            }
            Command::ZRangeUnified(
                ...
                let pairs = self.storage.zrange_unified(
                    &key,
                    &min,
                    &max,
                    by_score,
                ...
                for (member, score) in pairs {
                    ...
                    if with_scores {
                        resp_values.push(RespValue::BulkString(Some(Bytes::from(format!(
                            "{}",
                            score
                        )))));
                    }
            ...
            Command::Scan(cursor, pattern, count) => {
                executor_admin::execute_scan(self, cursor, pattern, count)
            }
            ...
            Command::Psync { replid, offset } => {
                ...
                if offset >= 0 && replid == master_replid {
                    ...
                    log::info!(
                        "PSYNC offset {} 不在 backlog 范围内，降级为全量同步",
                        offset
                    );
                }
            ...
            Command::ReplicaOf { host, port } => {
                ...
                tokio::spawn(async move {
                    if let Err(e) = repl_clone
                        .start_replication(storage_clone, host, port)
                        .await
                    {
                        log::error!("复制任务失败: {}", e);
                    }
