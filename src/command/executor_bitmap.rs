...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_bit_count(
    executor: &CommandExecutor,
    key: String,
    start: isize,
    end: isize,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_bit_field(
    executor: &CommandExecutor,
    key: String,
    ops: Vec<crate::storage::BitFieldOp>,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_bit_field_ro(
    executor: &CommandExecutor,
    key: String,
    ops: Vec<crate::storage::BitFieldOp>,
) -> Result<RespValue> {
