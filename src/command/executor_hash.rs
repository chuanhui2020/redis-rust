...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_str_len(
    executor: &CommandExecutor,
    key: String,
    field: String,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_m_get(
    executor: &CommandExecutor,
    key: String,
    fields: Vec<String>,
) -> Result<RespValue> {
    let values = executor.storage.hmget(&key, &fields)?;
    let resp_values: Vec<RespValue> = values.into_iter().map(RespValue::BulkString).collect();
    Ok(RespValue::Array(resp_values))
}
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_incr_by(
    executor: &CommandExecutor,
    key: String,
    field: String,
    delta: i64,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_incr_by_float(
    executor: &CommandExecutor,
    key: String,
    field: String,
    delta: f64,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_rand_field(
    executor: &CommandExecutor,
    key: String,
    count: i64,
    with_values: bool,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_p_expire(
    executor: &CommandExecutor,
    key: String,
    fields: Vec<String>,
    ms: u64,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_expire_at(
    executor: &CommandExecutor,
    key: String,
    fields: Vec<String>,
    ts: u64,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_p_expire_at(
    executor: &CommandExecutor,
    key: String,
    fields: Vec<String>,
    ts: u64,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_expire_time(
    executor: &CommandExecutor,
    key: String,
    fields: Vec<String>,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_p_expire_time(
    executor: &CommandExecutor,
    key: String,
    fields: Vec<String>,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_persist(
    executor: &CommandExecutor,
    key: String,
    fields: Vec<String>,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_h_get_del(
    executor: &CommandExecutor,
    key: String,
    fields: Vec<String>,
) -> Result<RespValue> {
