...
pub(crate) fn execute_x_add(
    ...
    match executor
        .storage
        .xadd(&key, &id, fields, nomkstream, max_len, min_id_parsed)?
    {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_rev_range(
    executor: &CommandExecutor,
    key: String,
    end: String,
    start: String,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_group_create(
    executor: &CommandExecutor,
    key: String,
    group: String,
    id: String,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_group_destroy(
    executor: &CommandExecutor,
    key: String,
    group: String,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_group_set_id(
    executor: &CommandExecutor,
    key: String,
    group: String,
    id: String,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_group_del_consumer(
    executor: &CommandExecutor,
    key: String,
    group: String,
    consumer: String,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_group_create_consumer(
    executor: &CommandExecutor,
    key: String,
    group: String,
    consumer: String,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_read_group(
    executor: &CommandExecutor,
    group: String,
    consumer: String,
    keys: Vec<String>,
...
pub(crate) fn execute_x_claim(
    ...
    let claimed = executor
        .storage
        .xclaim(&key, &group, &consumer, min_idle, &id_vec, justid)?;
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_auto_claim(
    executor: &CommandExecutor,
    key: String,
    group: String,
    consumer: String,
    ...
    let (next_id, claimed) = executor
        .storage
        .xautoclaim(&key, &group, &consumer, min_idle, start_id, count, justid)?;
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_pending(
    executor: &CommandExecutor,
    key: String,
    group: String,
    start: Option<String>,
    ...
    let start_id = match &start {
        Some(s) => {
            if s == "-" {
                Some(crate::storage::StreamId::new(0, 0))
            } else {
                Some(crate::storage::StreamId::parse(s)?)
            }
        }
        None => None,
    };
    let end_id = match &end {
        Some(s) => {
            if s == "+" {
                Some(crate::storage::StreamId::new(u64::MAX, u64::MAX))
            } else {
                Some(crate::storage::StreamId::parse(s)?)
            }
        }
    ...
    let (total, min_id, max_id, consumers, details) =
        executor
            .storage
            .xpending(&key, &group, start_id, end_id, count, consumer.as_deref())?;
    if start.is_none() {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_info_stream(
    executor: &CommandExecutor,
    key: String,
    full: bool,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_x_info_consumers(
    executor: &CommandExecutor,
    key: String,
    group: String,
) -> Result<RespValue> {
