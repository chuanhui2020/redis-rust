...
pub(crate) fn execute_set(
    ...
    key: String,
    value: Bytes,
    options: crate::storage::SetOptions,
) -> Result<RespValue> {
    if options.is_plain() {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_set_ex_cmd(
    executor: &CommandExecutor,
    key: String,
    value: Bytes,
    seconds: u64,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_incr_by_float(
    executor: &CommandExecutor,
    key: String,
    delta: f64,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_set_range(
    executor: &CommandExecutor,
    key: String,
    offset: usize,
    value: Bytes,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_get_range(
    executor: &CommandExecutor,
    key: String,
    start: i64,
    end: i64,
