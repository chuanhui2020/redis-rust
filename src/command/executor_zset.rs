...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_score(
    executor: &CommandExecutor,
    key: String,
    member: String,
) -> Result<RespValue> {
    match executor.storage.zscore(&key, &member)? {
        Some(score) => Ok(RespValue::BulkString(Some(Bytes::from(format!(
            "{}",
            score
        ))))),
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_range_by_score(
    executor: &CommandExecutor,
    key: String,
    min: f64,
    max: f64,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_rev_range(
    executor: &CommandExecutor,
    key: String,
    start: isize,
    stop: isize,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_rev_rank(
    executor: &CommandExecutor,
    key: String,
    member: String,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_incr_by(
    executor: &CommandExecutor,
    key: String,
    increment: f64,
    member: String,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_pop_min(
    executor: &CommandExecutor,
    key: String,
    count: usize,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_pop_max(
    executor: &CommandExecutor,
    key: String,
    count: usize,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_union_store(
    executor: &CommandExecutor,
    destination: String,
    keys: Vec<String>,
    weights: Vec<f64>,
    ...
    let weights_slice = if weights.is_empty() {
        None
    } else {
    ...
    let count = executor
        .storage
        .zunionstore(&destination, &keys, weights_slice, &aggregate)?;
    Ok(RespValue::Integer(count as i64))
}
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_inter_store(
    executor: &CommandExecutor,
    destination: String,
    keys: Vec<String>,
    weights: Vec<f64>,
    ...
    let count = executor
        .storage
        .zinterstore(&destination, &keys, weights_slice, &aggregate)?;
    Ok(RespValue::Integer(count as i64))
}
...
pub(crate) fn execute_z_scan(
    ...
    for (member, score) in items {
        ...
        item_values.push(RespValue::BulkString(Some(Bytes::from(format!(
            "{}",
            score
        )))));
    }
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_range_by_lex(
    executor: &CommandExecutor,
    key: String,
    min: String,
    max: String,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_inter_card(
    executor: &CommandExecutor,
    keys: Vec<String>,
    limit: usize,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_rem_range_by_lex(
    executor: &CommandExecutor,
    key: String,
    min: String,
    max: String,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_rem_range_by_rank(
    executor: &CommandExecutor,
    key: String,
    start: isize,
    stop: isize,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_rem_range_by_score(
    executor: &CommandExecutor,
    key: String,
    min: f64,
    max: f64,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_rand_member(
    executor: &CommandExecutor,
    key: String,
    count: i64,
    with_scores: bool,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_diff_store(
    executor: &CommandExecutor,
    destination: String,
    keys: Vec<String>,
) -> Result<RespValue> {
...
pub(crate) fn execute_z_inter(
    ...
    let pairs = executor
        .storage
        .zinter(&keys, weights_slice, &aggregate, with_scores)?;
    let mut resp_values = Vec::new();
    for (member, score) in pairs {
        resp_values.push(RespValue::BulkString(Some(Bytes::from(member))));
        if with_scores {
...
pub(crate) fn execute_z_union(
    ...
    let pairs = executor
        .storage
        .zunion(&keys, weights_slice, &aggregate, with_scores)?;
    let mut resp_values = Vec::new();
    for (member, score) in pairs {
        resp_values.push(RespValue::BulkString(Some(Bytes::from(member))));
        if with_scores {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_range_store(
    executor: &CommandExecutor,
    dst: String,
    src: String,
    min: String,
    ...
    let count = executor.storage.zrangestore(
        &dst,
        &src,
        &min,
        &max,
    ...
    match executor.storage.zmpop(&keys, min_or_max, count)? {
        Some((key, pairs)) => {
            ...
            for (member, score) in pairs {
                ...
                pair_values.push(RespValue::BulkString(Some(Bytes::from(format!(
                    "{}",
                    score
                )))));
            }
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_rev_range_by_score(
    executor: &CommandExecutor,
    key: String,
    max: f64,
    min: f64,
    ...
    let pairs = executor.storage.zrevrangebyscore(
        &key,
        max,
        min,
        with_scores,
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_rev_range_by_lex(
    executor: &CommandExecutor,
    key: String,
    max: String,
    min: String,
    ...
    let members = executor
        .storage
        .zrevrangebylex(&key, &max, &min, limit_offset, limit_count)?;
    let resp_values: Vec<RespValue> = members
        .into_iter()
        .map(|m| RespValue::BulkString(Some(Bytes::from(m))))
        .collect();
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_m_score(
    executor: &CommandExecutor,
    key: String,
    members: Vec<String>,
) -> Result<RespValue> {
...
/// - `Ok(RespValue::...)` - 执行成功
/// - `Err(AppError::...)` - 执行失败（键不存在、类型错误等）
pub(crate) fn execute_z_lex_count(
    executor: &CommandExecutor,
    key: String,
    min: String,
    max: String,
