//! 命令信息提取模块，从 Command 中提取命令名和 key 列表
...
pub(crate) fn extract_cmd_info(cmd: &Command) -> (String, Vec<String>) {
    match cmd {
        ...
        Command::LPushX(key, _) => ("LPUSHX".to_string(), vec![key.clone()]),
        Command::RPushX(key, _) => ("RPUSHX".to_string(), vec![key.clone()]),
        ...
        Command::HStrLen(key, field) => ("HSTRLEN".to_string(), vec![key.clone(), field.clone()]),
        ...
        Command::ZRemRangeByLex(key, _, _) => ("ZREMRANGEBYLEX".to_string(), vec![key.clone()]),
        Command::ZRemRangeByRank(key, _, _) => ("ZREMRANGEBYRANK".to_string(), vec![key.clone()]),
        Command::ZRemRangeByScore(key, _, _) => ("ZREMRANGEBYSCORE".to_string(), vec![key.clone()]),
        Command::ZScan(key, _, _, _) => ("ZSCAN".to_string(), vec![key.clone()]),
        Command::SInterCard(keys, _) => ("SINTERCARD".to_string(), keys.clone()),
        Command::ZInterCard(keys, _) => ("ZINTERCARD".to_string(), keys.clone()),
        ...
        Command::ClusterSlots => ("CLUSTER".to_string(), vec!["SLOTS".to_string()]),
        Command::ClusterShards => ("CLUSTER".to_string(), vec!["SHARDS".to_string()]),
        Command::ClusterMeet { .. } => ("CLUSTER".to_string(), vec!["MEET".to_string()]),
        Command::ClusterAddSlots(_) => ("CLUSTER".to_string(), vec!["ADDSLOTS".to_string()]),
