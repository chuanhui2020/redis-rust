...
impl CommandParser {
    ...
    fn parse_array(&self, arr: Vec<RespValue>) -> Result<Command> {
        ...
        match cmd_name.as_str() {
            ...
            "LPUSHX" => self.parse_lpushx(&arr),
            "RPUSHX" => self.parse_rpushx(&arr),
            ...
            "HSTRLEN" => self.parse_hstrlen(&arr),
            ...
            "HGETDEL" => self.parse_hgetdel(&arr),
            "HGETEX" => self.parse_hgetex(&arr),
            "HSETEX" => self.parse_hsetex(&arr),
            ...
            "ZREMRANGEBYLEX" => self.parse_zremrangebylex(&arr),
            "ZREMRANGEBYRANK" => self.parse_zremrangebyrank(&arr),
            "ZREMRANGEBYSCORE" => self.parse_zremrangebyscore(&arr),
            "SINTERCARD" => self.parse_sintercard(&arr),
            "ZINTERCARD" => self.parse_zintercard(&arr),
            ...
            "SORT_RO" => self.parse_sort_ro(&arr),
            "UNLINK" => self.parse_unlink(&arr),
            "COPY" => self.parse_copy(&arr),
            "MOVE" => self.parse_move(&arr),
    ...
    fn parse_sentinel(&self, arr: &[RespValue]) -> Result<Command> {
        ...
        match sub.as_str() {
            ...
            "IS-MASTER-DOWN-BY-ADDR" => {
                ...
                Ok(Command::SentinelIsMasterDownByAddr {
                    ip,
                    port,
                    current_epoch,
                    runid,
