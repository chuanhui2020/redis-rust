...
pub(crate) fn to_resp_set_bit(cmd: &Command) -> RespValue {
    match cmd {
        Command::SetBit(key, offset, value) => RespValue::Array(vec![
            bulk("SETBIT"),
            bulk(key),
...
pub(crate) fn to_resp_get_bit(cmd: &Command) -> RespValue {
    match cmd {
        Command::GetBit(key, offset) => {
            RespValue::Array(vec![bulk("GETBIT"), bulk(key), bulk(&offset.to_string())])
        }
        _ => unreachable!(),
    }
}
            ...
            for op in ops {
                match op {
                    crate::storage::BitFieldOp::Get(enc, off) => {
                        parts.push(bulk("GET"));
                        let type_str =
                            format!("{}{}", if enc.signed { "i" } else { "u" }, enc.bits);
