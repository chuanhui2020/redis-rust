...
pub(crate) fn to_resp_l_pop(cmd: &Command) -> RespValue {
    match cmd {
        Command::LPop(key) => RespValue::Array(vec![bulk("LPOP"), bulk(key)]),
        _ => unreachable!(),
    }
}
...
pub(crate) fn to_resp_r_pop(cmd: &Command) -> RespValue {
    match cmd {
        Command::RPop(key) => RespValue::Array(vec![bulk("RPOP"), bulk(key)]),
        _ => unreachable!(),
    }
}
...
pub(crate) fn to_resp_l_len(cmd: &Command) -> RespValue {
    match cmd {
        Command::LLen(key) => RespValue::Array(vec![bulk("LLEN"), bulk(key)]),
        _ => unreachable!(),
    }
}
...
pub(crate) fn to_resp_l_index(cmd: &Command) -> RespValue {
    match cmd {
        Command::LIndex(key, index) => {
            RespValue::Array(vec![bulk("LINDEX"), bulk(key), bulk(&index.to_string())])
        }
        _ => unreachable!(),
    }
}
...
pub(crate) fn to_resp_l_set(cmd: &Command) -> RespValue {
    match cmd {
        Command::LSet(key, index, value) => RespValue::Array(vec![
            bulk("LSET"),
            bulk(key),
...
pub(crate) fn to_resp_l_rem(cmd: &Command) -> RespValue {
    match cmd {
        Command::LRem(key, count, value) => RespValue::Array(vec![
            bulk("LREM"),
            bulk(key),
...
pub(crate) fn to_resp_l_trim(cmd: &Command) -> RespValue {
    match cmd {
        Command::LTrim(key, start, stop) => RespValue::Array(vec![
            bulk("LTRIM"),
            bulk(key),
            bulk(&start.to_string()),
            bulk(&stop.to_string()),
...
pub(crate) fn to_resp_l_pos(cmd: &Command) -> RespValue {
    match cmd {
        Command::LPos(key, value, rank, count, maxlen) => {
            let mut parts = vec![bulk("LPOS"), bulk(key), bulk_bytes(value)];
            if *rank != 1 {
