...
pub(crate) fn to_resp_x_len(cmd: &Command) -> RespValue {
    match cmd {
        Command::XLen(key) => RespValue::Array(vec![bulk("XLEN"), bulk(key)]),
        _ => unreachable!(),
    }
}
...
pub(crate) fn to_resp_x_trim(cmd: &Command) -> RespValue {
    match cmd {
        Command::XTrim(key, strategy, threshold) => RespValue::Array(vec![
            bulk("XTRIM"),
            bulk(key),
            bulk(strategy),
            bulk(threshold),
...
pub(crate) fn to_resp_x_set_id(cmd: &Command) -> RespValue {
    match cmd {
        Command::XSetId(key, id) => RespValue::Array(vec![bulk("XSETID"), bulk(key), bulk(id)]),
        _ => unreachable!(),
    }
}
...
pub(crate) fn to_resp_x_group_destroy(cmd: &Command) -> RespValue {
    match cmd {
        Command::XGroupDestroy(key, group) => RespValue::Array(vec![
            bulk("XGROUP"),
            bulk("DESTROY"),
            bulk(key),
            bulk(group),
...
pub(crate) fn to_resp_x_group_set_id(cmd: &Command) -> RespValue {
    match cmd {
        Command::XGroupSetId(key, group, id) => RespValue::Array(vec![
            bulk("XGROUP"),
            bulk("SETID"),
            bulk(key),
            bulk(group),
...
pub(crate) fn to_resp_x_group_del_consumer(cmd: &Command) -> RespValue {
    match cmd {
        Command::XGroupDelConsumer(key, group, consumer) => RespValue::Array(vec![
            bulk("XGROUP"),
            bulk("DELCONSUMER"),
            bulk(key),
            bulk(group),
...
pub(crate) fn to_resp_x_group_create_consumer(cmd: &Command) -> RespValue {
    match cmd {
        Command::XGroupCreateConsumer(key, group, consumer) => RespValue::Array(vec![
            bulk("XGROUP"),
            bulk("CREATECONSUMER"),
            bulk(key),
            bulk(group),
    ...
    match cmd {
        Command::XClaim(key, group, consumer, min_idle, ids, justid) => {
            let mut parts = vec![
                ...
                bulk(group),
                bulk(consumer),
                bulk(&min_idle.to_string()),
            ];
            for id in ids {
                parts.push(bulk(id));
            }
            if *justid {
                parts.push(bulk("JUSTID"));
            }
            RespValue::Array(parts)
        }
        _ => unreachable!(),
...
pub(crate) fn to_resp_x_info_consumers(cmd: &Command) -> RespValue {
    match cmd {
        Command::XInfoConsumers(key, group) => RespValue::Array(vec![
            bulk("XINFO"),
            bulk("CONSUMERS"),
            bulk(key),
            bulk(group),
