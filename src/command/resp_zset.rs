...
pub(crate) fn to_resp_z_card(cmd: &Command) -> RespValue {
    match cmd {
        Command::ZCard(key) => RespValue::Array(vec![bulk("ZCARD"), bulk(key)]),
        _ => unreachable!(),
    }
}
...
pub(crate) fn to_resp_z_incr_by(cmd: &Command) -> RespValue {
    match cmd {
        Command::ZIncrBy(key, increment, member) => RespValue::Array(vec![
            bulk("ZINCRBY"),
            bulk(key),
...
pub(crate) fn to_resp_z_count(cmd: &Command) -> RespValue {
    match cmd {
        Command::ZCount(key, min, max) => RespValue::Array(vec![
            bulk("ZCOUNT"),
            bulk(key),
            bulk(&min.to_string()),
            bulk(&max.to_string()),
...
pub(crate) fn to_resp_z_scan(cmd: &Command) -> RespValue {
    match cmd {
        Command::ZScan(key, cursor, pattern, count) => {
            let mut parts = vec![bulk("ZSCAN"), bulk(key), bulk(&cursor.to_string())];
...
pub(crate) fn to_resp_z_rem_range_by_lex(cmd: &Command) -> RespValue {
    match cmd {
        Command::ZRemRangeByLex(key, min, max) => RespValue::Array(vec![
            bulk("ZREMRANGEBYLEX"),
            bulk(key),
            bulk(min),
            bulk(max),
...
pub(crate) fn to_resp_z_rem_range_by_rank(cmd: &Command) -> RespValue {
    match cmd {
        Command::ZRemRangeByRank(key, start, stop) => RespValue::Array(vec![
...
pub(crate) fn to_resp_z_rem_range_by_score(cmd: &Command) -> RespValue {
    match cmd {
        Command::ZRemRangeByScore(key, min, max) => RespValue::Array(vec![
