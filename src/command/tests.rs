...
fn test_parse_exists() {
    ...
    let cmd = parser.parse(resp).unwrap();
    assert_eq!(cmd, Command::Exists(vec!["x".to_string(), "y".to_string()]));
}
...
fn test_execute_set_get() {
    ...
    assert_eq!(resp, RespValue::BulkString(Some(Bytes::from("redis"))));
}
...
fn test_execute_get_nonexistent() {
    ...
    let resp = executor
        .execute(Command::Get("missing".to_string()))
        .unwrap();
...
fn test_execute_expire_ttl() {
    ...
    let resp = executor.execute(Command::Ttl("key".to_string())).unwrap();
...
fn test_empty_key_and_value() {
    ...
    assert_eq!(resp, RespValue::BulkString(Some(Bytes::from_static(b""))));
}
...
fn test_execute_append() {
    ...
    let resp = executor
        .execute(Command::Append("s".to_string(), Bytes::from("hello")))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(5));

    let resp = executor
        .execute(Command::Append("s".to_string(), Bytes::from(" world")))
        .unwrap();
...
fn test_execute_setnx() {
    ...
    let resp = executor
        .execute(Command::SetNx("key".to_string(), Bytes::from("first")))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));

    let resp = executor
        .execute(Command::SetNx("key".to_string(), Bytes::from("second")))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(0));

    let resp = executor.execute(Command::Get("key".to_string())).unwrap();
    assert_eq!(resp, RespValue::BulkString(Some(Bytes::from("first"))));
}
...
fn test_execute_strlen() {
    ...
    let resp = executor
        .execute(Command::StrLen("missing".to_string()))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(0));
...
fn test_parse_incr_decr_family() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["INCRBY", "k", "5"]))
            .unwrap(),
        Command::IncrBy("k".to_string(), 5)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["DECRBY", "k", "3"]))
            .unwrap(),
...
fn test_parse_append_setnx_getrange_strlen() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["APPEND", "k", "v"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["GETRANGE", "k", "0", "3"]))
            .unwrap(),
...
fn test_execute_lrange_lindex() {
    ...
    let resp = executor
        ...
        .unwrap();
    assert_eq!(resp, RespValue::BulkString(Some(Bytes::from("d"))));
...
fn test_execute_list_wrongtype() {
    ...
    let result = executor.execute(Command::LPush("s".to_string(), vec![Bytes::from("x")]));
    assert!(result.is_err());
...
fn test_parse_list_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["LPUSH", "k", "v1", "v2"]))
            .unwrap(),
        Command::LPush("k".to_string(), vec![Bytes::from("v1"), Bytes::from("v2")])
    );
    assert_eq!(
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["LRANGE", "k", "0", "-1"]))
            .unwrap(),
        Command::LRange("k".to_string(), 0, -1)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["LINDEX", "k", "2"]))
            .unwrap(),
...
fn test_execute_hset_hget() {
    ...
    let resp = executor
        ...
        .unwrap();
    assert_eq!(resp, RespValue::BulkString(Some(Bytes::from("redis2"))));
}
...
fn test_execute_hdel_hexists_hlen() {
    ...
    let resp = executor.execute(Command::HLen("hash".to_string())).unwrap();
    assert_eq!(resp, RespValue::Integer(2));

    let resp = executor
        .execute(Command::HDel("hash".to_string(), vec!["a".to_string()]))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));
...
fn test_execute_hash_wrongtype() {
    ...
    let result = executor.execute(Command::HGet("s".to_string(), "field".to_string()));
    assert!(result.is_err());
}
...
fn test_parse_hash_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["HSET", "k", "f", "v"]))
            .unwrap(),
        Command::HSet("k".to_string(), vec![("f".to_string(), Bytes::from("v"))])
    );
    assert_eq!(
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["HDEL", "k", "f1", "f2"]))
            .unwrap(),
        Command::HDel("k".to_string(), vec!["f1".to_string(), "f2".to_string()])
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["HEXISTS", "k", "f"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["HMSET", "k", "f", "v"]))
            .unwrap(),
        Command::HMSet("k".to_string(), vec![("f".to_string(), Bytes::from("v"))])
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["HMGET", "k", "f1", "f2"]))
            .unwrap(),
        Command::HMGet("k".to_string(), vec!["f1".to_string(), "f2".to_string()])
    );
}
...
fn test_execute_sadd_srem_smembers() {
    ...
    let resp = executor
        .execute(Command::SIsMember("set".to_string(), Bytes::from("a")))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));

    let resp = executor.execute(Command::SCard("set".to_string())).unwrap();
    assert_eq!(resp, RespValue::Integer(3));

    let resp = executor
        .execute(Command::SRem("set".to_string(), vec![Bytes::from("a")]))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));
...
fn test_execute_set_ops() {
    ...
    let resp = executor
        .execute(Command::SInter(vec!["a".to_string(), "b".to_string()]))
        .unwrap();
    match resp {
        RespValue::Array(arr) => assert_eq!(arr.len(), 2),
        other => panic!("期望 Array，得到 {:?}", other),
    ...
    let resp = executor
        .execute(Command::SUnion(vec!["a".to_string(), "b".to_string()]))
        .unwrap();
    match resp {
    ...
    let resp = executor
        .execute(Command::SDiff(vec!["a".to_string(), "b".to_string()]))
        .unwrap();
    match resp {
...
fn test_execute_set_wrongtype() {
    ...
    let result = executor.execute(Command::SAdd("s".to_string(), vec![Bytes::from("x")]));
    assert!(result.is_err());
...
fn test_parse_set_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["SADD", "k", "m1", "m2"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["SISMEMBER", "k", "m"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["SINTER", "a", "b"]))
            .unwrap(),
        Command::SInter(vec!["a".to_string(), "b".to_string()])
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["SUNION", "a", "b"]))
            .unwrap(),
...
fn test_execute_zadd_zscore_zrank() {
    ...
    let resp = executor
        ...
        .unwrap();
    assert_eq!(resp, RespValue::BulkString(Some(Bytes::from("20"))));
...
fn test_execute_zset_wrongtype() {
    ...
    let result = executor.execute(Command::ZAdd("s".to_string(), vec![(1.0, "x".to_string())]));
    assert!(result.is_err());
...
fn test_parse_zset_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZADD", "k", "1", "a", "2", "b"]))
            .unwrap(),
        Command::ZAdd(
            "k".to_string(),
            vec![(1.0, "a".to_string()), (2.0, "b".to_string())]
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZREM", "k", "a", "b"]))
            .unwrap(),
        Command::ZRem("k".to_string(), vec!["a".to_string(), "b".to_string()])
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZSCORE", "k", "a"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZRANGE", "k", "0", "-1"]))
            .unwrap(),
        Command::ZRange("k".to_string(), 0, -1, false)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZRANGE", "k", "0", "-1", "WITHSCORES"]))
            .unwrap(),
        Command::ZRange("k".to_string(), 0, -1, true)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZRANGEBYSCORE", "k", "0", "100"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZREVRANGE", "k", "0", "-1"]))
            .unwrap(),
        Command::ZRevRange("k".to_string(), 0, -1, false)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZREVRANK", "k", "a"]))
            .unwrap(),
        Command::ZRevRank("k".to_string(), "a".to_string())
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZINCRBY", "k", "1.5", "a"]))
            .unwrap(),
        Command::ZIncrBy("k".to_string(), 1.5, "a".to_string())
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZCOUNT", "k", "0", "100"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZPOPMAX", "k", "2"]))
            .unwrap(),
        Command::ZPopMax("k".to_string(), 2)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&[
                "ZUNIONSTORE",
                "d",
                "2",
                "a",
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZINTERSTORE", "d", "1", "a"]))
            .unwrap(),
        Command::ZInterStore(
            "d".to_string(),
            vec!["a".to_string()],
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["ZRANGEBYLEX", "k", "[a", "[z"]))
            .unwrap(),
        Command::ZRangeByLex("k".to_string(), "[a".to_string(), "[z".to_string())
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["SETBIT", "k", "7", "1"]))
            .unwrap(),
        Command::SetBit("k".to_string(), 7, true)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["GETBIT", "k", "7"]))
            .unwrap(),
        Command::GetBit("k".to_string(), 7)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["BITCOUNT", "k", "0", "-1", "BYTE"]))
            .unwrap(),
        Command::BitCount("k".to_string(), 0, -1, true)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["BITOP", "AND", "dest", "a", "b"]))
            .unwrap(),
        Command::BitOp(
            "AND".to_string(),
            "dest".to_string(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["BITPOS", "k", "1", "0", "-1", "BYTE"]))
            .unwrap(),
...
fn test_execute_setbit_getbit() {
    ...
    let resp = executor
        .execute(Command::SetBit("k".to_string(), 0, true))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(0));

    let resp = executor
        .execute(Command::SetBit("k".to_string(), 0, false))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));

    let resp = executor
        .execute(Command::GetBit("k".to_string(), 0))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(0));

    let resp = executor
        .execute(Command::GetBit("k".to_string(), 100))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(0));
}
...
fn test_execute_bitcount() {
    ...
    executor
        .execute(Command::Set(
            "k".to_string(),
            Bytes::from(vec![0b10101010, 0b11110000]),
            crate::storage::SetOptions::default(),
        ))
        .unwrap();

    let resp = executor
        .execute(Command::BitCount("k".to_string(), 0, -1, true))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(8));

    let resp = executor
        .execute(Command::BitCount("k".to_string(), 0, 0, true))
        .unwrap();
...
fn test_execute_bitop() {
    ...
    executor
        .execute(Command::Set(
            "b".to_string(),
            Bytes::from(vec![0b10101010]),
            crate::storage::SetOptions::default(),
        ))
        .unwrap();

    let resp = executor
        .execute(Command::BitOp(
            "AND".to_string(),
            "dest".to_string(),
            vec!["a".to_string(), "b".to_string()],
        ))
    ...
    assert_eq!(
        resp,
        RespValue::BulkString(Some(Bytes::from(vec![0b10100000])))
    );
}
...
fn test_execute_bitpos() {
    ...
    let resp = executor
        .execute(Command::BitPos("k".to_string(), 1, 0, -1, true))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(0));

    let resp = executor
        .execute(Command::BitPos("k".to_string(), 0, 0, -1, true))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));
}
...
fn test_parse_geo_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["GEOADD", "k", "116.4", "39.9", "北京"]))
            .unwrap(),
        Command::GeoAdd("k".to_string(), vec![(116.4, 39.9, "北京".to_string())])
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["GEODIST", "k", "a", "b", "km"]))
            .unwrap(),
        Command::GeoDist(
            "k".to_string(),
            "a".to_string(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["GEOHASH", "k", "a"]))
            .unwrap(),
        Command::GeoHash("k".to_string(), vec!["a".to_string()])
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["GEOPOS", "k", "a"]))
            .unwrap(),
...
fn test_parse_georadius_commands() {
    ...
    // GEORADIUS 基础解析
    let cmd = parser
        .parse(make_bulk_array(&["GEORADIUS", "k", "0", "0", "100", "km"]))
        .unwrap();
    match cmd {
        Command::GeoRadius(
            key,
    ...
    // WAITAOF
    let cmd = parser
        .parse(make_bulk_array(&["WAITAOF", "1", "0", "1000"]))
        .unwrap();
    match cmd {
        Command::WaitAof {
            numlocal,
...
fn test_execute_georadius_and_waitaof() {
    ...
    // GEORADIUSBYMEMBER
    let resp = executor
        .execute(Command::GeoRadiusByMember(
            "cities".to_string(),
            "北京".to_string(),
            2000000.0,
            "m".to_string(),
    ...
    // GEORADIUSBYMEMBER_RO
    let resp = executor
        .execute(Command::GeoRadiusByMemberRo(
            "cities".to_string(),
            "北京".to_string(),
            2000000.0,
            "m".to_string(),
...
fn test_execute_pfadd_pfcount() {
    ...
    let resp = executor
        .execute(Command::PfCount(vec!["hll".to_string()]))
        .unwrap();
    match resp {
...
fn test_parse_hll_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PFADD", "k", "a", "b"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PFCOUNT", "k1", "k2"]))
            .unwrap(),
        Command::PfCount(vec!["k1".to_string(), "k2".to_string()])
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PFMERGE", "dest", "k1", "k2"]))
            .unwrap(),
...
fn test_execute_zunionstore() {
    ...
    executor
        .execute(Command::ZAdd(
            "z2".to_string(),
            vec![(2.0, "b".to_string()), (3.0, "c".to_string())],
        ))
        .unwrap();
...
fn test_execute_keys_scan_rename_type_persist_pexpire_pttl_dbsize_info() {
    ...
    // SCAN
    let resp = executor
        .execute(Command::Scan(0, "*".to_string(), 10))
        .unwrap();
    match resp {
        RespValue::Array(arr) => {
    ...
    let resp = executor
        .execute(Command::PExpire("temp".to_string(), 5000))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));
...
fn test_parse_key_management_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["SCAN", "0", "MATCH", "a*", "COUNT", "5"]))
            .unwrap(),
        Command::Scan(0, "a*".to_string(), 5)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["RENAME", "a", "b"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PEXPIRE", "k", "1000"]))
            .unwrap(),
...
fn test_parse_pubsub_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["SUBSCRIBE", "ch1", "ch2"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["UNSUBSCRIBE", "ch1"]))
            .unwrap(),
        Command::Unsubscribe(vec!["ch1".to_string()])
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PUBLISH", "ch1", "hello"]))
            .unwrap(),
        Command::Publish("ch1".to_string(), Bytes::from("hello"))
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PSUBSCRIBE", "news.*"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PUBSUB", "CHANNELS"]))
            .unwrap(),
        Command::PubSubChannels(None)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PUBSUB", "CHANNELS", "news.*"]))
            .unwrap(),
        Command::PubSubChannels(Some("news.*".to_string()))
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PUBSUB", "NUMSUB"]))
            .unwrap(),
        Command::PubSubNumSub(vec![])
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PUBSUB", "NUMSUB", "news", "weather"]))
            .unwrap(),
        Command::PubSubNumSub(vec!["news".to_string(), "weather".to_string()])
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PUBSUB", "NUMPAT"]))
            .unwrap(),
...
fn test_parse_config_maxmemory() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["CONFIG", "GET", "maxmemory"]))
            .unwrap(),
        Command::ConfigGet("maxmemory".to_string())
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["CONFIG", "SET", "maxmemory", "1024"]))
            .unwrap(),
...
fn test_parse_string_tail_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["SETEX", "k", "10", "v"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["PSETEX", "k", "100", "v"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["GETSET", "k", "v"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["GETEX", "k", "EX", "10"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["GETEX", "k", "PX", "100"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["GETEX", "k", "PERSIST"]))
            .unwrap(),
        Command::GetEx("k".to_string(), GetExOption::Persist)
    );
    // MSETNX
    assert_eq!(
        parser
            .parse(make_bulk_array(&["MSETNX", "a", "1", "b", "2"]))
            .unwrap(),
        Command::MSetNx(vec![
            ("a".to_string(), Bytes::from("1")),
            ("b".to_string(), Bytes::from("2")),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["INCRBYFLOAT", "k", "0.5"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["SETRANGE", "k", "6", "Redis"]))
            .unwrap(),
...
fn test_execute_string_tail_commands() {
    ...
    // SETEX + GET
    executor
        .execute(Command::SetExCmd("k1".to_string(), Bytes::from("v1"), 3600))
        .unwrap();
    ...
    executor
        ...
        .unwrap();
    let resp = executor
        .execute(Command::GetSet("k3".to_string(), Bytes::from("new")))
        .unwrap();
    ...
    // GETEX PERSIST
    executor
        .execute(Command::SetExCmd("k5".to_string(), Bytes::from("v5"), 3600))
        .unwrap();
    let resp = executor
        .execute(Command::GetEx("k5".to_string(), GetExOption::Persist))
        .unwrap();
    ...
    // INCRBYFLOAT
    let resp = executor
        .execute(Command::IncrByFloat("f".to_string(), 0.5))
        .unwrap();
    assert_eq!(resp, RespValue::BulkString(Some(Bytes::from("0.5"))));
    let resp = executor
        .execute(Command::IncrByFloat("f".to_string(), 0.25))
        .unwrap();
    ...
    executor
        ...
        .unwrap();
    let resp = executor
        .execute(Command::SetRange("s".to_string(), 6, Bytes::from("Redis")))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(11));
    let resp = executor.execute(Command::Get("s".to_string())).unwrap();
    assert_eq!(
...
fn test_parse_list_tail_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["LSET", "k", "1", "v"]))
            .unwrap(),
        Command::LSet("k".to_string(), 1, Bytes::from("v"))
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["LINSERT", "k", "BEFORE", "p", "v"]))
            .unwrap(),
        Command::LInsert(
            "k".to_string(),
            crate::storage::LInsertPosition::Before,
            Bytes::from("p"),
            Bytes::from("v")
        )
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["LINSERT", "k", "AFTER", "p", "v"]))
            .unwrap(),
        Command::LInsert(
            "k".to_string(),
            crate::storage::LInsertPosition::After,
            Bytes::from("p"),
            Bytes::from("v")
        )
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["LREM", "k", "2", "v"]))
            .unwrap(),
        Command::LRem("k".to_string(), 2, Bytes::from("v"))
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["LTRIM", "k", "1", "3"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&[
                "LPOS", "k", "v", "RANK", "2", "COUNT", "3", "MAXLEN", "100"
            ]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["BLPOP", "k1", "k2", "5"]))
            .unwrap(),
        Command::BLPop(vec!["k1".to_string(), "k2".to_string()], 5.0)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["BRPOP", "k1", "k2", "0"]))
            .unwrap(),
...
fn test_execute_list_tail_commands() {
    ...
    executor
        ...
        .unwrap();
    let resp = executor
        .execute(Command::LRem("list2".to_string(), 1, Bytes::from("a")))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));
    ...
    let resp = executor
        .execute(Command::BLPop(vec!["empty".to_string()], 1.0))
        .unwrap();
    assert_eq!(resp, RespValue::BulkString(None));
    ...
    executor
        ...
        .unwrap();
    let resp = executor
        .execute(Command::BLPop(vec!["blist".to_string()], 1.0))
        .unwrap();
    match resp {
        RespValue::Array(arr) => {
            assert_eq!(arr.len(), 2);
...
fn test_parse_hash_tail_commands() {
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["HINCRBY", "k", "f", "5"]))
            .unwrap(),
        Command::HIncrBy("k".to_string(), "f".to_string(), 5)
    );
    assert_eq!(
        parser
            .parse(make_bulk_array(&["HINCRBYFLOAT", "k", "f", "0.5"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["HSETNX", "k", "f", "v"]))
            .unwrap(),
    ...
    assert_eq!(
        parser
            .parse(make_bulk_array(&["HRANDFIELD", "k", "2", "WITHVALUES"]))
            .unwrap(),
...
fn test_execute_hash_tail_commands() {
    ...
    // HINCRBY
    let resp = executor
        .execute(Command::HIncrBy("h".to_string(), "f".to_string(), 5))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(5));
    let resp = executor
        .execute(Command::HIncrBy("h".to_string(), "f".to_string(), 3))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(8));

    // HINCRBYFLOAT
    let resp = executor
        .execute(Command::HIncrByFloat("h".to_string(), "g".to_string(), 0.5))
        .unwrap();
    assert_eq!(resp, RespValue::BulkString(Some(Bytes::from("0.5"))));
    ...
    executor
        ...
        .unwrap();
    let resp = executor
        .execute(Command::HScan("h4".to_string(), 0, "*".to_string(), 2))
        .unwrap();
    match resp {
        RespValue::Array(arr) => {
...
fn test_execute_set_tail_commands() {
    ...
    executor
        ...
        .unwrap();
    let resp = executor
        .execute(Command::SPop("s1".to_string(), 2))
        .unwrap();
    match resp {
        RespValue::Array(arr) => {
            assert_eq!(arr.len(), 2);
    ...
    executor
        ...
        .unwrap();
    let resp = executor
        .execute(Command::SRandMember("s2".to_string(), 2))
        .unwrap();
    match resp {
        RespValue::Array(arr) => {
            assert_eq!(arr.len(), 2);
    ...
    executor
        ...
        .unwrap();
    executor
        .execute(Command::SAdd("s4".to_string(), vec![Bytes::from("c")]))
        .unwrap();
    let resp = executor
        .execute(Command::SMove(
            "s3".to_string(),
            "s4".to_string(),
            Bytes::from("a"),
        ))
    ...
    executor
        ...
        .unwrap();
    let resp = executor
        .execute(Command::SScan("s7".to_string(), 0, "*".to_string(), 2))
        .unwrap();
    match resp {
        RespValue::Array(arr) => {
            assert_eq!(arr.len(), 2); // cursor + array
...
fn test_parse_auth() {
    ...
    assert_eq!(
        cmd,
        Command::Auth("default".to_string(), "mypassword".to_string())
    );
}
...
fn test_parse_copy() {
    ...
    assert_eq!(
        cmd,
        Command::Copy("src".to_string(), "dest".to_string(), true)
    );
}
...
fn test_parse_restore() {
    ...
    assert_eq!(
        cmd,
        Command::Restore("key".to_string(), 0, b"data".to_vec(), false)
    );
}
...
fn test_execute_copy() {
    ...
    let resp = executor
        .execute(Command::Copy("src".to_string(), "dest".to_string(), false))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));
...
fn test_parse_eval() {
    ...
    assert_eq!(
        cmd,
        Command::Eval("return 1".to_string(), Vec::new(), Vec::new())
    );
}
...
fn test_parse_evalsha() {
    ...
    assert_eq!(
        cmd,
        Command::EvalSha("abc123".to_string(), Vec::new(), Vec::new())
    );
}
...
fn test_parse_script_exists() {
    ...
    assert_eq!(
        cmd,
        Command::ScriptExists(vec!["abc".to_string(), "def".to_string()])
    );
}
...
fn test_execute_eval() {
    ...
    let resp = executor
        .execute(Command::Eval("return 42".to_string(), vec![], vec![]))
        .unwrap();
...
fn test_execute_script_load_exists_flush() {
    ...
    let load_resp = executor
        .execute(Command::ScriptLoad("return 1".to_string()))
        .unwrap();
    ...
    let exists_resp = executor
        .execute(Command::ScriptExists(vec![sha1.clone()]))
        .unwrap();
    match exists_resp {
...
fn test_to_resp_value_save_bgsave() {
    assert_eq!(
        Command::Save.to_resp_value(),
        RespValue::Array(vec![bulk("SAVE")])
    );
    assert_eq!(
        Command::BgSave.to_resp_value(),
...
fn test_execute_expire_at() {
    ...
    let now = std::time::SystemTime::now()
        ...
        .as_secs();
    let resp = executor
        .execute(Command::ExpireAt("k".to_string(), now + 10))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));
    let resp2 = executor
        .execute(Command::ExpireTime("k".to_string()))
        .unwrap();
...
fn test_execute_pexpire_at() {
    ...
    let now = std::time::SystemTime::now()
        ...
        .as_millis() as u64;
    let resp = executor
        .execute(Command::PExpireAt("k".to_string(), now + 10000))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(1));
    let resp2 = executor
        .execute(Command::PExpireTime("k".to_string()))
        .unwrap();
    assert!(matches!(resp2, RespValue::Integer(t) if t >= now as i64));
}
...
fn test_execute_renamenx() {
    ...
    executor
        ...
        .unwrap();
    let resp = executor
        .execute(Command::RenameNx("a".to_string(), "b".to_string()))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(0));
    let resp2 = executor
        .execute(Command::RenameNx("a".to_string(), "c".to_string()))
        .unwrap();
...
fn test_execute_swapdb() {
    ...
    assert_eq!(
        ...
        RespValue::BulkString(Some(Bytes::from("db1")))
    );
}
...
fn test_execute_substr() {
    ...
    executor
        ...
        .unwrap();
    let resp = executor
        .execute(Command::SubStr("k".to_string(), 0, 4))
        .unwrap();
    assert_eq!(resp, RespValue::BulkString(Some(Bytes::from("hello"))));
}
...
fn test_set_nx_fail() {
    ...
    assert_eq!(
        executor.execute(Command::Get("k".to_string())).unwrap(),
        RespValue::BulkString(Some(Bytes::from("old")))
    );
}
...
fn test_set_keepttl() {
    let executor = CommandExecutor::new(StorageEngine::new());
    executor
        .execute(Command::SetEx("k".to_string(), Bytes::from("old"), 10000))
        .unwrap();
...
fn test_set_nx_get() {
    ...
    let opts2 = crate::storage::SetOptions {
        nx: true,
        get: true,
        ..Default::default()
    };
    let resp2 = executor
        .execute(Command::Set("k".to_string(), Bytes::from("new"), opts2))
        .unwrap();
...
fn test_storage_lcs() {
    ...
    engine
        .set("k1".to_string(), Bytes::from("hello world"))
        .unwrap();
    engine
        .set("k2".to_string(), Bytes::from("hello rust"))
        .unwrap();
...
fn test_parse_fcall() {
    ...
    let cmd = parser.parse(resp).unwrap();
    assert!(
        matches!(cmd, Command::FCall(name, keys, args) if name == "myfunc" && keys == vec!["key1"] && args == vec!["arg1"])
    );
}
...
fn test_execute_function_delete() {
    ...
    let del_resp = executor
        .execute(Command::FunctionDelete("mylib".to_string()))
        .unwrap();
    assert_eq!(del_resp, RespValue::Integer(1));

    let del_resp2 = executor
        .execute(Command::FunctionDelete("mylib".to_string()))
        .unwrap();
...
fn test_execute_function_dump_restore() {
    ...
    executor
        .execute(Command::FunctionRestore(dump_data, "FLUSH".to_string()))
        .unwrap();
    let list_after = executor
        .execute(Command::FunctionList(None, false))
        .unwrap();
    match list_after {
        RespValue::Array(arr) => assert_eq!(arr.len(), 1),
...
fn test_execute_eval_ro() {
    ...
    let resp = executor
        .execute(Command::EvalRO("return 42".to_string(), vec![], vec![]))
        .unwrap();
    assert_eq!(resp, RespValue::Integer(42));
}
...
fn test_execute_fcall_ro() {
    ...
    let resp = executor
        .execute(Command::FCallRO("readonly_fn".to_string(), vec![], vec![]))
        .unwrap();
...
fn test_parse_client_kill() {
    let parser = CommandParser::new();
    let cmd = parser
        .parse(make_bulk_array(&["CLIENT", "KILL", "ID", "42"]))
        .unwrap();
...
fn test_parse_client_pause() {
    let parser = CommandParser::new();
    let cmd = parser
        .parse(make_bulk_array(&["CLIENT", "PAUSE", "100", "WRITE"]))
        .unwrap();
...
fn test_parse_client_unpause() {
    let parser = CommandParser::new();
    let cmd = parser
        .parse(make_bulk_array(&["CLIENT", "UNPAUSE"]))
        .unwrap();
...
fn test_parse_client_no_evict() {
    let parser = CommandParser::new();
    let cmd = parser
        .parse(make_bulk_array(&["CLIENT", "NO-EVICT", "ON"]))
        .unwrap();
    assert!(matches!(cmd, Command::ClientNoEvict(true)));

    let cmd = parser
        .parse(make_bulk_array(&["CLIENT", "NO-EVICT", "OFF"]))
        .unwrap();
...
fn test_parse_client_no_touch() {
    let parser = CommandParser::new();
    let cmd = parser
        .parse(make_bulk_array(&["CLIENT", "NO-TOUCH", "ON"]))
        .unwrap();
...
fn test_parse_client_reply() {
    let parser = CommandParser::new();
    let cmd = parser
        .parse(make_bulk_array(&["CLIENT", "REPLY", "OFF"]))
        .unwrap();
    assert!(matches!(
        cmd,
        Command::ClientReply(crate::server::ReplyMode::Off)
    ));

    let cmd = parser
        .parse(make_bulk_array(&["CLIENT", "REPLY", "SKIP"]))
        .unwrap();
    assert!(matches!(
        cmd,
        Command::ClientReply(crate::server::ReplyMode::Skip)
    ));
}
...
fn test_parse_client_unblock() {
    let parser = CommandParser::new();
    let cmd = parser
        .parse(make_bulk_array(&["CLIENT", "UNBLOCK", "42", "ERROR"]))
        .unwrap();
