...
fn is_read_command(cmd: &Command) -> bool {
    matches!(
        ...
        Command::Get(_)
            ...
            | Command::ZRangeByScore(_, _, _, _)
            | Command::ZRevRangeByScore(_, _, _, _, _, _)
            | Command::ZLexCount(_, _, _)
            | Command::ZRangeByLex(_, _, _)
            | Command::ZRevRangeByLex(_, _, _, _, _)
            | Command::ZMScore(_, _)
            | Command::XLen(_)
            | Command::XRange(_, _, _, _)
            | Command::XRevRange(_, _, _, _)
            | Command::Ping(_)
            | Command::DbSize
            | Command::Info(_)
            ...
            | Command::Scan(_, _, _)
            | Command::SScan(_, _, _, _)
            | Command::ZScan(_, _, _, _)
            | Command::GetBit(_, _)
            | Command::BitCount(_, _, _, _)
            | Command::BitPos(_, _, _, _, _)
            | Command::PfCount(_)
    )
}
...
fn extract_tracking_keys(cmd: &Command) -> Vec<String> {
    ...
    match cmd {
        ...
        | ZRevRank(k, _)
        | ZRange(k, _, _, _)
        | ZRangeByScore(k, _, _, _)
        | ZLexCount(k, _, _)
        | ZRangeByLex(k, _, _)
        | ZRevRangeByLex(k, _, _, _, _)
        | ZMScore(k, _)
        | XLen(k)
        | XRange(k, _, _, _)
        | XRevRange(k, _, _, _)
...
fn build_cluster_links(cluster: &crate::cluster::ClusterState) -> Vec<RespValue> {
    ...
    for node in cluster.get_nodes() {
        ...
        let direction = "to";
        let events = if node.flags.contains(&crate::cluster::state::NodeFlag::PFail)
            || node.flags.contains(&crate::cluster::state::NodeFlag::Fail)
        {
            "r"
        } else {
            "rw"
...
fn build_cluster_shards(cluster: &crate::cluster::ClusterState) -> Vec<RespValue> {
    ...
    for node in cluster.get_nodes() {
        if !node
            .flags
            .contains(&crate::cluster::state::NodeFlag::Master)
        {
            continue;
        }
    ...
    for key in keys {
        ...
        let resp = loop {
            match parser.parse(&mut buf) {
                Ok(Some(r)) => break r,
                Ok(None) => {
                    let n = tokio::time::timeout(timeout, target_stream.read_buf(&mut buf))
                        .await
...
fn sentinel_instance_to_resp(inst: &crate::sentinel::SentinelInstance) -> RespValue {
    RespValue::Array(vec![
        ...
        RespValue::BulkString(Some(Bytes::from(if inst.sdown {
            "s_down,master"
        } else {
            "master"
        }))),
...
pub(crate) async fn handle_connection(
    ...
    let mut executor = match aof.clone() {
        Some(aof_writer) => CommandExecutor::new_with_aof(storage.clone(), aof_writer),
                        ...
                        Ok(cmd) => {
                            ...
                            if matches!(
                                ...
                                let resp = if let Some(s) = sentinel.as_ref() {
                                    match cmd {
                                                    ...
                                                    let arr: Vec<RespValue> = m
                                                        ...
                                                        .map(|r| {
                                                            RespValue::Array(vec![
                                                                ...
                                                                RespValue::BulkString(Some(
                                                                    Bytes::from(r.ip.clone()),
                                                                )),
                                                                RespValue::BulkString(Some(
                                                                    Bytes::from("port"),
                                                                )),
                                                                RespValue::BulkString(Some(
                                                                    Bytes::from(r.port.to_string()),
                                                                )),
                                                                RespValue::BulkString(Some(
                                                                    Bytes::from("flags"),
                                                                )),
                                                                RespValue::BulkString(Some(
                                                                    Bytes::from(if r.sdown {
                                                                        "slave,s_down"
                                                                    } else {
                                                                        "slave"
                                                                    }),
                                                    ...
                                                    let arr: Vec<RespValue> = m
                                                        ...
                                                        .map(|s| {
                                                            RespValue::Array(vec![
                                                                ...
                                                                RespValue::BulkString(Some(
                                                                    Bytes::from(s.ip.clone()),
                                                                )),
                                                                RespValue::BulkString(Some(
                                                                    Bytes::from("port"),
                                                                )),
                                                                RespValue::BulkString(Some(
                                                                    Bytes::from(s.port.to_string()),
                                                                )),
                                                                RespValue::BulkString(Some(
                                                                    Bytes::from("runid"),
                                                                )),
                                                                RespValue::BulkString(Some(
                                                                    Bytes::from(s.runid.clone()),
                                                                )),
                                                            ])
                                                        })
                                                        .collect();
                                        ...
                                        Command::SentinelMyId => RespValue::BulkString(Some(
                                            Bytes::from(s.runid.clone()),
                                        )),
                                        Command::SentinelIsMasterDownByAddr {
                                            ip,
                                            ...
                                            let down_state = matched
                                                .map_or(0i64, |m| if m.sdown { 1 } else { 0 });
                                            ...
                                            if let Some(master) = matched {
                                                ...
                                                if runid != "*" && !runid.is_empty() {
                                                    let (voted, voted_runid) = s.vote_for(
                                                        &master.name,
                                                        current_epoch,
                                                        &runid,
                                                    );
                            ...
                            if matches!(
                                ...
                                let resp = if let Some(c) = cluster.as_ref() {
                                    match cmd {
                                        Command::ClusterInfo => RespValue::BulkString(Some(
                                            Bytes::from(c.get_info_string()),
                                        )),
                                        Command::ClusterNodes => RespValue::BulkString(Some(
                                            Bytes::from(format_cluster_nodes(c)),
                                        )),
                                        Command::ClusterMyId => {
                                        ...
                                        Command::ClusterReplicate(node_id) => {
                                            ...
                                            if node_id == my_id {
                                                ...
                                                let (master_ip, master_port) =
                                                    if let Some(target) = c.get_node(&node_id) {
                                                ...
                                                if let Some(node) = nodes.get_mut(&my_id) {
                                                    ...
                                                    if !node.flags.contains(
                                                        &crate::cluster::state::NodeFlag::Slave,
                                                    ) {
                                                        node.flags.push(
                                                            crate::cluster::state::NodeFlag::Slave,
                                                        );
                                                    ...
                                                    node.slots = vec![
                                                            false;
                                                            crate::cluster::state::CLUSTER_SLOTS
                                                        ];
                                                }
                                                drop(nodes);
                                                ...
                                                if node.flags.contains(
                                                    ...
                                                    if let Some(ref master_id) = node.master_id {
                                                        if c.promote_replica_to_master(master_id) {
                                                            ...
                                                            log::warn!(
                                                                "CLUSTER FAILOVER: 本节点已提升为 master，接管 master {} 的 slot，epoch={}",
                                                                master_id,
                                                                epoch
                                                            );
                                                            // 广播拓扑更新
                                            ...
                                            match storage.keys("*") {
                                                Ok(keys) => {
                                                    let count = keys
                                                        .iter()
                                                        .filter(|k| {
                                                            crate::cluster::ClusterState::key_slot(
                                                                k,
                                                            ) == slot
                                                        })
                                                        .count();
                                        ...
                                        Command::ClusterCountFailureReports(node_id) => {
                                            let count = if let Some(node) = c.get_node(&node_id) {
                                                let mut cnt = 0;
                                                if node.flags.contains(
                                                    &crate::cluster::state::NodeFlag::PFail,
                                                ) {
                                                    cnt += 1;
                                                }
                                                if node.flags.contains(
                                                    &crate::cluster::state::NodeFlag::Fail,
                                                ) {
                                                    cnt += 1;
                                                }
                                                cnt
                            ...
                            if !is_repl_cmd {
                                if let Some(ref cluster) = cluster {
                                    ...
                                    if let Some(first_key) = keys.first() {
                                            ...
                                            && node_id != my_id
                                        {
                                            ...
                                            let allow_readonly =
                                                readonly_mode && is_read_command(&cmd) && {
                                                    let myself = cluster.myself();
                                                    myself
                                                        .map(|n| {
                                                            n.master_id.as_deref()
                                                                == Some(node_id.as_str())
                                                        })
                                                        .unwrap_or(false)
                                                };
                                ...
                                if let Some(ref _cluster) = cluster {
                                    ...
                                    if keys.len() > 1 {
                                        let first_slot =
                                            crate::cluster::ClusterState::key_slot(&keys[0]);
                            ...
                            match cmd {
                                    ...
                                    let resp = if let Some(ref aof) = aof {
                                        ...
                                        match path {
                                            Some(p) => {
                                                ...
                                                let use_rdb_preamble =
                                                    handler.executor.aof_use_rdb_preamble();
                                                match crate::aof::AofRewriter::rewrite(
                                                    &storage,
                                                    &temp_path,
                                                    &p,
                                                    use_rdb_preamble,
                                ...
                                | Command::PubSubShardChannels(_)
                                | Command::PubSubShardNumSub(_) => {
                                ...
                                Command::ClientTrackingInfo => {
                                    ...
                                    if tracking_enabled {
                                        flags_arr
                                            .push(RespValue::BulkString(Some(Bytes::from("on"))));
                                    } else {
                                        flags_arr
                                            .push(RespValue::BulkString(Some(Bytes::from("off"))));
                                    }
                                    if tracking_bcast {
                                        flags_arr.push(RespValue::BulkString(Some(Bytes::from(
                                            "bcast",
                                        ))));
                                    }
                                    if tracking_optin {
                                    ...
                                    for p in &tracking_prefixes {
                                        prefixes_arr.push(RespValue::BulkString(Some(
                                            Bytes::from(p.clone()),
                                        )));
                                    }
                                    let resp = RespValue::Array(vec![
                                        ...
                                        RespValue::Integer(
                                            tracking_redirect.map(|r| r as i64).unwrap_or(-1),
                                        ),
                                    ...
                                    match handler.executor.execute(cmd) {
                                        Ok(RespValue::BulkString(None)) => {
                                                ...
                                                .await
                                            {
                                                Ok(Some((key, value))) => {
                                                    // 阻塞成功，执行 LPOP 写 AOF
                                                    handler.executor.record_applied_write(
                                ...
                                Command::Save => match rdb_path {
                                    Some(ref path) => {
                                        let repl_info = replication.as_ref().map(|repl| {
                                            (
                                                repl.get_master_replid(),
                                                repl.get_master_repl_offset(),
                                            )
                                        });
                                    ...
                                    match handler.executor.execute(cmd) {
                                        Ok(RespValue::BulkString(None)) => {
                                            match handler
                                                .executor
                                                .storage()
                                                .blmove(&source, &dest, left_from, left_to, timeout)
                                                .await
                                            {
                                                Ok(Some(value)) => {
                                                    handler.executor.record_applied_write(
                                                ...
                                                .await
                                            {
                                                Ok(Some((key, values))) => {
                                                    ...
                                                    for _ in 0..record_count {
                                                        let cmd = if left {
                                                            Command::LPop(record_key.clone())
                                                        } else {
                                                            Command::RPop(record_key.clone())
                                                        };
                                                        handler.executor.record_applied_write(&cmd);
                                                    }
                                                    let mut arr = Vec::new();
                                                    arr.push(RespValue::BulkString(Some(
                                                        bytes::Bytes::from(key),
                                                    )));
                                    ...
                                    match handler.executor.execute(cmd) {
                                        Ok(RespValue::BulkString(None)) => {
                                            match handler
                                                .executor
                                                .storage()
                                                .bzmpop(&keys, min_or_max, count, timeout)
                                                .await
                                            {
                                                Ok(Some((key, pairs))) => {
                                                    let record_cmd = if min_or_max {
                                                        Command::ZPopMin(key.clone(), pairs.len())
                                                    } else {
                                                        Command::ZPopMax(key.clone(), pairs.len())
                                                    };
                                                    handler
                                                        .executor
                                                        .record_applied_write(&record_cmd);
                                                    let mut pair_values = Vec::new();
                                                    for (member, score) in pairs {
                                                        pair_values.push(RespValue::BulkString(
                                                            Some(bytes::Bytes::from(member)),
                                                        ));
                                                        pair_values.push(RespValue::BulkString(
                                ...
                                Command::BZPopMin(keys, timeout) => {
                                    ...
                                    match handler.executor.execute(cmd) {
                                        Ok(RespValue::Array(ref arr)) if !arr.is_empty() => {
                                            let key = &keys[0];
                                            let resp = RespValue::Array(vec![
                                                RespValue::BulkString(Some(bytes::Bytes::from(
                                                    key.clone(),
                                                ))),
                                                arr[0].clone(),
                                                arr[1].clone(),
                                        ...
                                        Ok(migrated_keys) => {
                                            ...
                                            if !copy {
                                                for key in &migrated_keys {
                                                    if let Err(e) =
                                                        handler.executor.storage().del(key)
                                                    {
                                                        log::warn!(
                                                            "MIGRATE 后删除本地 key {} 失败: {}",
                                                            key,
                                                            e
                                                        );
                                                    }
                                        ...
                                        Some(repl) => {
                                            if !matches!(
                                                ...
                                                if replicas.is_empty() {
                                                    ...
                                                    log::info!(
                                                        "FAILOVER 开始: 自动选择副本, timeout={}ms, force={}",
                                                        timeout,
                                                        force
                                                    );
                                                    RespValue::SimpleString("OK".to_string())
                                ...
                                other => {
                                    ...
                                    if let Command::ReplConf { args } = &other
                                        && args.len() >= 2
                                        && args[0].to_uppercase() == "LISTENING-PORT"
                                        ...
                                        Ok(response) => {
                                                ...
                                                && s.starts_with("CONTINUE")
                                            {
                                                ...
                                                if parts.len() >= 3 {
                                                    ...
                                                    if let Some(ref repl) = replication {
                                                        if let Some(backlog_data) =
                                                            repl.get_backlog_from_offset(offset)
                                                            && let Err(e) = stream
                                                                .write_all(&backlog_data)
                                                                .await
                                                        {
                                                        ...
                                                        let _ = stream.flush().await;
                                                        return run_replica_forward_loop(
                                                            stream.into_inner(),
                                                            repl,
                                                            &peer_addr,
                                                            replica_listening_port,
                                                        )
                                                        .await;
                                                    }
                                                }
                                                ...
                                                && s.starts_with("FULLRESYNC")
                                            {
                                                ...
                                                let repl_id = replication
                                                    .as_ref()
                                                    .map(|repl| repl.get_master_replid());
                                                let repl_offset = replication
                                                    .as_ref()
                                                    .map(|repl| repl.get_master_repl_offset());
                                                ...
                                                if let Some(ref repl) = replication {
                                                    ...
                                                    if let Some(backlog_data) = repl
                                                        .get_backlog_from_offset(
                                                            repl_offset.unwrap_or(0),
                                                        )
                                                        && let Err(e) =
                                                            stream.write_all(&backlog_data).await
                                                    {
