    ...
    pub fn xreadgroup(
        ...
        for (idx, key) in keys.iter().enumerate() {
            ...
            if read_new {
                ...
                for (id, fields) in stream.entries.range((
                                    ...
                                    if idle >= min_idle_time {
                                        ...
                                        if old_consumer_name != consumer_name
                                            && let Some(old_consumer) =
                                                group.consumers.get_mut(&old_consumer_name)
                                        {
                ...
                if v.is_expired() {
                    ...
                    match &mut v.value {
                        StorageValue::Stream(s) => {
                            ...
                            let pel_ids: Vec<StreamId> = group
                                .pel
                                .keys()
                                .filter(|id| **id >= start)
                                .copied()
...
self.bump_version(&mut map, key);
            ...
            Some(v) => {
                ...
                match &v.value {
                    StorageValue::Stream(s) => {
                        ...
                        // 消费者统计
                        let mut consumer_counts: std::collections::HashMap<String, usize> =
    ...
    pub fn xinfo_stream(
        ...
        key: &str,
        _full: bool,
    ) -> Result<Option<(usize, usize, StreamId, StreamId, Vec<String>)>> {
        let db = self.db();
        let map = db
            .inner
            .get_shard(key)
    ...
    pub fn xinfo_consumers(
        ...
        key: &str,
        group_name: &str,
    ) -> Result<Vec<(String, usize, u64, u64)>> {
        let db = self.db();
        let map = db
            .inner
            .get_shard(key)
